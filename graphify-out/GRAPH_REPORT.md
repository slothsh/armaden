...
## Corpus Check
- 211 files · ~41,457 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2091 nodes · 4054 edges · 276 communities (74 shown, 202 thin omitted)
- Extraction: 81% EXTRACTED · 19% INFERRED · 0% AMBIGUOUS · INFERRED: 765 edges (avg confidence: 0.55)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `d9c2ea81`
...
## Communities (276 total, 202 thin omitted)

### Community 0 - "ArmaReforgerServerExecutable"
Cohesion: 0.02
Nodes (58): ArmaReforgerServerExecutable, Path to a server configuration JSON file., Redirect log output to the given directory., Bind the server to specific addresses / ports.          Keyword Args:, Configure A2S query endpoint.          Keyword Args:             address: IP add, Configure BattlEye / RCON remote console.          Keyword Args:             add, Cap the server frame rate., Load a server-side addon (mod) by ID.          May be called multiple times to l (+50 more)

### Community 2 - "AuthManager"
Cohesion: 0.06
Nodes (29): GenericError, DefaultApiError, A2SConfig, GameConfig, GamePropertiesConfig, GamePropertiesPersistence, JoinQueueConfig, ModConfig (+21 more)

### Community 6 - "BattleEyeRconServer"
Cohesion: 0.06
Nodes (13): AbstractEventLoop, InstanceContainer, ServiceProvider, ApplicationException, CoreApplication, Any, Result, ServiceProvider (+5 more)
...
Cohesion: 0.05
Nodes (21): HttpKernel, Middleware, MiddlewarePipeline, json_response(), JSONResponse, response(), ResponseFactory, DefaultApi (+13 more)
...
- **Why does `ArmaReforgerServerExecutable` connect `ArmaReforgerServerExecutable` to `AuthManager`, `SteamCmdExecutable`, `ServiceProvider`, `TaskRuntimeInterface`, `.no_backend`, `.profile`, `.rpl_encode_as_long_jobs`, `.server_world`, `.silent_crash_report`, `.staggering_budget`, `.streaming_budget`, `CommandResponse`, `.world`, `.jobsys_short_worker_count`, `api.py`, `.log_scr_checksum`, `Executable`, `.disable_crash_reporter`, `.force_session_load`, `.jobsys_short_worker_count`, `.keep_session_save`, `.log_rdb_checksum`, `.nds`?**
  _High betweenness centrality (0.130) - this node is a cross-community bridge._
- **Why does `InstanceContainer` connect `InstanceContainer` to `DefaultApplication`, `.get_alias`, `WorkerPool`, `RconCommandArgumentError`, `RconCommandInterface`, `ContextualAttribute`, `.resolve_primitive`, `BattleEyeRconClient`, `SubprocessHandle`, `.backend_local_storage`, `Exception`, `get_application`, `Message`, `Kernel`, `.resolve`, `.log_append`, `HttpServiceProvider`?**
  _High betweenness centrality (0.103) - this node is a cross-community bridge._
- **Why does `TaskRuntimeInterface` connect `CommandResponse` to `ArmaReforgerServerExecutable`, `AuthManager`, `RestartPolicy`, `TaskThreadingPolicy`, `._initialize_configs`, `ServiceProvider`, `TaskGraph`, `Dictionary`, `DatagramTransportInterface`?**
